...
public class UserController extends ControllerBase {
    ...
    public ResponseEntity<?> entry(@PathVariable String preEntryId,
            @RequestBody AccountCreateInformation accountCreateInformation) {
        try {
            userService.entry(preEntryId, accountCreateInformation.getUserId(), accountCreateInformation.getName(),
                    accountCreateInformation.getPassword());
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            return exceptionProcess(e);
        }
    ...
    public ResponseEntity<?> updateUser(@RequestHeader("Authorization") String authHeader,
            @RequestBody AccountUpdateInformation accountUpdateInformation) {
        try {
            String accessToken = pickupAccessToken(authHeader);
    ...
    @PatchMapping("/api/v1/user/password/{userId}")
    ...
    public ResponseEntity<?> deleteUserForce(
        ...
        try {
            if (deleteReason == null || deleteReason.isEmpty())
                return ResponseEntity.badRequest().body("Delete reason is required");
            String accessToken = pickupAccessToken(authHeader);
